...
placeshot:
	...
	mulu.w	#40,d4
	ror.l	#3,d3			;divide by 8 for byt
	add.w	d3,d4
	move.w	#0,d3
	rol.l	#3,d3
	lea	shot,a4
...
enemy2:
	...
	move.b	(a4)+,d6
	ror.w	d3,d6			;first part of data
	move.b	0(a3,d4.w),d2		;reareœö(d from screen
	or.b	d6,d2
	move.b	d2,0(a3,d4.w)		;write to screen
...
;	cmp.w	#160,0(a2,d0.w)		;compare y value to top of screen  
;	bne.s	continue4		;if not past, continue
;	move.w	#0,0(a0,d0.w)		;clear hit value
...
loopdown:
	cmp.w	#0,0(a0,d0.w)		;check a0 hit value
	beq.s	nodowning
	add.w	#2,0(a2,d0.w)		;add 1 fro€(*um y value	
	cmp.w	#170,0(a2,d0.w)		;compare y value to top of screen  
	bne.s	nodowning		;if not past, continue
	move.w	#0,0(a0,d0.w)		;clear hit value
nodowning:
	subq.w	#2,d0
	cmp.w	#-2,d0
...
check1:
	...
	sub.w	d2,d4			; sub from shot Xuxy`(=bpl	positive_x		
	neg.w	d4
positive_x:
