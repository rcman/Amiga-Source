...
Forbid		equ	-30-102
Permit		equ	-30-108
AllocMem	equ	-30-168 ;ByteSize,Requirements/d0,d1
FreeMem		equ	-30-180 ;MemoryBlock,ByteSize/a1,d0
...
Execbase	equ	4
Planesize	equ	80*200	;Bitplane size :80 bytes by 200 lines
Planewidth	equ	80
CLsize		equ	3*4	;The Copper-List contains 2\3 commands
Chip		equ	2 	;Allocate Chip-RAM
Clear		equ	Chip+$98000 ; Clear Chip-RAM first
...
; Allocate Memrory for bit plane
	...
	move.l	Execbase,a6
	move.l	#98000,d0	;Memory requirment for bit plane
	move.l	#$10002,d1	
	jsr	AllocMem(a6)	;Allocate Memory
	move.l	d0,Planeadr
;	beq	Ende		;Error The end
...
; Determine start values:
	...
	move.w	#255,a3		;Size od Bitplane in Register
	move.w	#639,a4
	move.w	#$0303,d7	;Start pattern

Loop:
	rol.w	#2,d7		;Shift pattern
	move.w	d7,a2		;Pattern in regualar for Drawline
